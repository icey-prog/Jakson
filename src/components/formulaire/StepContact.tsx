  ...
  return (
    <div className="space-y-5">
      ...
      <div>
        ...
        <p className="text-[11px] text-apple-ink-48 mt-1">Format attendu : +226 ou numéro local</p>
      ...
      <div className="flex items-start gap-3">
        <button
          ...
          className={`mt-0.5 w-[18px] h-[18px] rounded-sm border-2 flex items-center justify-center shrink-0 transition-all duration-200 cursor-pointer ${
            formData.optin ? 'bg-jackson-vivid border-jackson-vivid' : 'border-apple-ink-48/50 bg-white'
          }`}
        >
          {formData.optin && <Check size={12} className="text-white" />}
        </button>
        <span className="text-[13px] text-apple-ink-48 leading-relaxed">
          J&apos;accepte de recevoir mon devis et d&apos;être contacté par un conseiller Jackson Assurances.
        </span>
      </div>
      {errors.optin && <p className="text-xs text-red-600">{errors.optin}</p>}
      ...
      <div className="flex justify-between pt-2">
        ...
        <button
          onClick={onSubmit}
          disabled={loading}
          className="px-8 py-3.5 bg-apple-blue hover:bg-apple-blue-focus text-white font-semibold rounded-btn transition-all duration-250 disabled:opacity-70 disabled:cursor-not-allowed flex items-center gap-2 cursor-pointer shadow-[0_8px_32px_rgba(20,184,166,0.2)]"
        >
          {loading
            ? <span className="w-5 h-5 border-2 border-apple-blue/40 border-t-white rounded-full animate-spin" />
